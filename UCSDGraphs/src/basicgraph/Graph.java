...
public abstract class Graph {
	...
	/**
	 * descend the order of the  List<Integer> type.
	 */
	private class ListIntegerComparator implements Comparator<Integer>{
	...
	public List<Integer> degreeSequence() {
		...
		for(int i = 0; i < this.getNumVertices(); i++){
			//计算该节点的初度与入度，相加为degree
			degrees.add(this.getNeighbors(i).size() + this.getInNeighbors(i).size());
		}
		degrees.sort(new ListIntegerComparator());
		return degrees;
	}
