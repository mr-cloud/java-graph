...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		int[][] distance2Array = new int[this.getNumVertices()][this.getNumVertices()];
		//三重循环，完成矩阵相乘
		for(int i = 0; i < this.getNumVertices(); i++){
			for(int j = 0; j < this.getNumVertices(); j++){
				for(int k = 0; k < this.getNumVertices(); k++){
					distance2Array[i][j] += this.adjMatrix[i][k]*this.adjMatrix[k][j];
				}
			}
		}
		//遍历v的出度，找到two-hop点，加入list
		for(int i = 0; i < this.getNumVertices(); i++){
			for(int j = 0; j < distance2Array[v][i]; j++){
				twoHops.add(i);
			}
		}
		return twoHops;
